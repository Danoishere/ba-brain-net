...
class VisionNet(nn.Module):
    def __init__(self):
        ...
        self.rnn = nn.LSTM(1024, self.hidden_dim, self.n_layers)
    ...
    def forward(self, frame):
        ...
        out_fc1 = self.fc1(frame)
        out = self.lrelu(out_fc1)
        out = out.reshape(1, batch_size, -1)
        out, hidden = self.rnn(out, self.hidden)
        self.hidden = hidden
        out = out.reshape(batch_size, -1)
        out = self.lrelu(out)
        out = torch.cat((out, out_fc1), 1)
        out = self.fc2(out)
        return out
...
class ClassToPosNet(nn.Module):
    ...
    def loss(self, y_pred_pos, y_target_pos_t):
        ...
        #diff_sum_sqrt = ((torch.sqrt(diff)*10)**2)*0.1

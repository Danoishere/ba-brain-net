...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, torchDevice, input_size, input_dim, hidden_dim, kernel_size, bias):
        """
    ...
    def init_hidden(self, batch_size, cuda=True):
        state = (Variable(torch.zeros(batch_size, self.hidden_dim, self.height, self.width).to(self.torchDevice)),
                 Variable(torch.zeros(batch_size, self.hidden_dim, self.height, self.width).to(self.torchDevice)))
        return state


class ConvLSTM(nn.Module):

    def __init__(self, torchDevice, input_size, input_dim, hidden_dim, kernel_size, num_layers,
        ...
        for i in range(0, self.num_layers):
            ...
            cell_list.append(ConvLSTMCell(self.torchDevice, input_size=(self.height, self.width),
